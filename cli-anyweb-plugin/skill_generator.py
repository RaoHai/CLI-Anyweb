"""
SKILL.md Generator for cli-anyweb

This module extracts metadata from cli-anyweb harnesses and generates
...
@dataclass
class SkillMetadata:
    """Metadata extracted from a cli-anyweb harness."""
    skill_name: str
...
def extract_cli_metadata(harness_path: str) -> SkillMetadata:
    """
    Extract metadata from a cli-anyweb harness directory.
    ...
    # Find the cli_anyweb/<software> directory
    cli_anyweb_dir = harness_path / "cli_anyweb"
    if not cli_anyweb_dir.exists():
        raise ValueError(
            f"cli_anyweb directory not found in {harness_path}. "
            "Ensure the harness structure includes cli_anyweb/<software>/"
        )
    software_dirs = [d for d in cli_anyweb_dir.iterdir()
    ...
    skill_name = f"cli-anyweb-{software_name}"
...
def generate_examples(software_name: str, command_groups: list[CommandGroup]) -> list[Example]:
    ...
    examples.append(Example(
        ...
        code=f"""cli-anyweb-{software_name} project new -o myproject.json
# Or with JSON output for programmatic use
cli-anyweb-{software_name} --json project new -o myproject.json"""
    ))
    ...
    examples.append(Example(
        ...
        code=f"""cli-anyweb-{software_name}
    ...
    for group in command_groups:
        if "export" in group.name.lower():
            examples.append(Example(
                ...
                code=f"""cli-anyweb-{software_name} --project myproject.json export render output.pdf --overwrite"""
            ))
            break
...
def generate_skill_md_simple(metadata: SkillMetadata) -> str:
    ...
    lines = [
        ...
        "## Installation",
        "",
        f"This CLI is installed as part of the cli-anyweb-{metadata.software_name} package:",
        "",
        f"```bash",
        f"pip install cli-anyweb-{metadata.software_name}",
        f"```",
        "",
        "**Prerequisites:**",
        "- Python 3.10+",
    ...
    lines.extend([
        ...
        "```bash",
        "# Show help",
        f"cli-anyweb-{metadata.software_name} --help",
        "",
        "# Start interactive REPL mode",
        f"cli-anyweb-{metadata.software_name}",
        "",
        "# Create a new project",
        f"cli-anyweb-{metadata.software_name} project new -o project.json",
        "",
        "# Run with JSON output (for agent consumption)",
        f"cli-anyweb-{metadata.software_name} --json project info -p project.json",
        "```",
        "",
    ])
...
def generate_skill_file(harness_path: str, output_path: Optional[str] = None,
    ...
    """
    Generate a SKILL.md file for a cli-anyweb harness.

    Args:
        harness_path: Path to the agent-harness directory
        output_path: Optional output path for SKILL.md (default: cli_anyweb/<software>/skills/SKILL.md)
    ...
    if output_path is None:
        ...
        output_path = harness_path_obj / "cli_anyweb" / metadata.software_name / "skills" / "SKILL.md"
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate SKILL.md for cli-anyweb harnesses"
    )
    parser.add_argument(
        "harness_path",
    ...
    parser.add_argument(
        "-o", "--output",
        help="Output path for SKILL.md (default: cli_anyweb/<software>/skills/SKILL.md)",
        default=None
    )
    parser.add_argument(
        "-t", "--template",
