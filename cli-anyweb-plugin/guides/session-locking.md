...
Never use bare `open("w") + json.dump()` 鈥?`open("w")` truncates the file before any lock can be acquired.
...
def _locked_save_json(path, data, **dump_kwargs) -> None:
    ...
    try:
        ...
        f = open(path, "w")             # first save 鈥?file doesn't exist yet
    with f:
        _locked = False
        try:
            import fcntl
            ...
            pass                        # Windows / unsupported FS 鈥?proceed unlocked
        try:
            f.seek(0)
