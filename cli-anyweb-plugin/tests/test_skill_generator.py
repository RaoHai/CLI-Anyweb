...
# - In the repo: cli-anyweb-plugin/skill_generator.py (parent dir)
...
def harness_dir(tmp_path):
    ...
    software = "testapp"
    cli_pkg = tmp_path / "cli_anyweb" / software
...
class TestExtractCliMetadata:
    ...
    def test_extracts_skill_name(self, harness_dir):
        metadata = extract_cli_metadata(str(harness_dir))
        assert metadata.skill_name == "cli-anyweb-testapp"
    ...
    def test_raises_on_missing_cli_anyweb_dir(self, tmp_path):
        with pytest.raises(ValueError, match="cli_anyweb directory not found"):
            extract_cli_metadata(str(tmp_path))

    def test_raises_on_empty_cli_anyweb_dir(self, tmp_path):
        (tmp_path / "cli_anyweb").mkdir()
...
class TestGenerateSkillMd:
    ...
    def test_simple_output_has_yaml_frontmatter(self):
        ...
        assert 'name: "cli-anyweb-testapp"' in content
    ...
    def test_simple_output_has_installation_section(self):
        ...
        assert "pip install cli-anyweb-testapp" in content
    ...
    def test_simple_output_has_examples(self):
        examples = [
            Example(
                ...
                code="cli-anyweb-testapp --help",
            )
        ]
        metadata = self._make_metadata(examples=examples)
        content = generate_skill_md_simple(metadata)
        assert "### Quick Start" in content
        assert "cli-anyweb-testapp --help" in content
    ...
    def test_generate_skill_md_falls_back_to_simple(self):
        ...
        assert "cli-anyweb-testapp" in content
...
class TestEdgeCases:
    def test_harness_without_readme(self, tmp_path):
        ...
        assert metadata.skill_intro == ""  # No README 鈫?empty intro
