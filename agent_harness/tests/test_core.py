...
class TestAgentBrowserBackend:
    """Test direct backend helpers added for the flat CLI surface."""
    ...
    def test_get_value_url(self):
        ...
        with patch("agent_harness.utils.agent_browser_backend._get_current_url") as mock_get_url:
            ...
            assert result == {"field": "url", "value": "https://example.com"}

    def test_get_value_rejects_unknown_field(self):
        """Unknown get fields raise a clear error."""
        with pytest.raises(ValueError, match="Unsupported field"):
            backend.get_value("title")
    ...
    def test_find_wraps_grep_matches_with_count(self):
        ...
        with patch("agent_harness.utils.agent_browser_backend.grep") as mock_grep:
            ...
            assert result["matches"] == ["/main/button[0]", "/main/link[0]"]
